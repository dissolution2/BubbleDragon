...
public class EnemyB extends SmallEnemyDef {
    ...
    //private StateMachine<EnemyB, EnemyBState> stateMachine;
    ...
    private float actionFlyby_Height;
    private boolean enemyHasSteeringStatBool;

    private Array<EnemyBullet> enemyBulletArray;

    // this is new take a better look later on this way to implement super
    public EnemyB(PlayScreen screen, float x, float y, MapObject object, GameManagerAssets instance) { //}, int idOfEnemyFromTiledMap) {
        super(screen, x, y);
        ...
        frames.add(new TextureRegion( GameUtility.getTextureAtlas("spriteAtlas/BubbleEnemyFlyingAtlas.atlas").findRegion("EnemyB"), 0, 0, 64, 64));
        ...
        this.enemyBulletArray = new Array<EnemyBullet>();
        ...
        this.actionFlyby_Height = 0;

        this.enemyHasSteeringStatBool = false;
...
//System.out.println( "EnemyA Class updateAllSpawnLifeFromEnemy: State Machine getPre state : " + stateMachine.getPreviousState() );
    ...
    RayCastCallback rayCallback = new RayCastCallback() {
        ...
        public float reportRayFixture(Fixture fixture, Vector2 point, Vector2 normal, float fraction) {
            ...
            if( fixture.getBody().getType() != BodyType.StaticBody ) {
                ...
                collision.set(point);
                EnemyB.this.normal.set(normal).add(point);
    ...
    public void fireEnemyBullet(int power){

        switch (power){
            case 1:
                if(!(enemyBulletArray.size > 0) ){

                    enemyBulletArray.add(new EnemyBullet(world, b2body.getPosition().x, b2body.getPosition().y, 1, runningRight ? true : false));
                }
                break;
        }
    ...
    protected void defineEnemy() {
        ...
        GameUtility.GROUND_BIT |
                ...
                .WALL_AND_SEALING_BIT |
         */

        fdef.filter.maskBits = GameUtility.GAME_AI_OBJECT_BIT |
...
*/
    ...
    private float flyingUpTimer = 0;
    private float flyingDownTimer = 0;
    ...
    */
    //test
    float flybyTestTimer = 0;
    float hitTimer = 0;
    public void update(float dt) {

        if(gameManagerAssetsInstance.getGameState() != GameManagerAssets.GameState.GAME_PAUSED ) {

            //System.out.println("EnemyB gravity is " + this.b2body.getGravityScale());
            ...
            /*
            EnemyB gravity is 1.0
            body mass 1.0
            world Gravity (0.0,-10.0)
            body WorldCenter (22.09,10.62)
             */
/*
            System.out.println("body mass " + this.b2body.getMass());
            System.out.println("world Gravity " + world.getGravity() );
            System.out.println("body WorldCenter " + this.b2body.getWorldCenter());
*/
            //this.b2body.setGravityScale(0);
...
/*
            if(!runningRight){
                this.b2body.setLinearVelocity(new Vector2(-0.9f, -0.4f));
                //System.out.println("EnemyB vel -1.2f -0.4f");

                flyingUpTimer += dt;
                if(flyingUpTimer > 0.8f){
                    this.b2body.setGravityScale(0.002f + flyingUpTimer);
                    this.b2body.setLinearVelocity(new Vector2(-0.9f, 0.4f));
                    flyingDownTimer += dt;
                }

                if(flyingDownTimer > 0.5f){
                    this.b2body.setGravityScale(-0.8f + (-flyingDownTimer));
                    this.b2body.setLinearVelocity(new Vector2(-0.9f, -0.4f));
                    //System.out.println("EnemyB vel -1.2f -0.4f");
                }

                if(flyingDownTimer > 0.8f){
                    flyingUpTimer =0;
                    flyingDownTimer =0;
                }
            ...
            if(runningRight){
                ...
                flyingUpTimer += dt;
                if(flyingUpTimer > 0.8f){
                    this.b2body.setGravityScale(0.2f + flyingUpTimer);
                    this.b2body.setLinearVelocity(new Vector2(0.9f, 0.4f));
                    flyingDownTimer += dt;
                }

                if(flyingDownTimer > 0.5f){
                    this.b2body.setGravityScale(-1.2f + (-flyingDownTimer));
                    this.b2body.setLinearVelocity(new Vector2(0.9f, -0.4f));
                    //System.out.println("EnemyB vel -1.2f -0.4f");
                }
...
*/
            ...
            if (setToDestroy && !destroyed) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    System.out.println("Error on rayCasting EnemyB: " + e + " enemy ID: " + this.enemyID );
                }

                if (!isDead()) {

                    if(!enemyHasSteeringStatBool) {
                        if(!runningRight){
                            this.b2body.setLinearVelocity(new Vector2(-0.9f, -0.4f));
                            //System.out.println("EnemyB vel -1.2f -0.4f");
                            ...
                            if(flyingUpTimer > 0.8f){
                                this.b2body.setGravityScale(0.002f + flyingUpTimer);
                                this.b2body.setLinearVelocity(new Vector2(-0.9f, 0.2f)); // from 0.4 to 0.2  // 0.8 showing good upRise
                                flyingDownTimer += dt;
                            }
...
*/
            ...
            for(EnemyBullet  ball : enemyBulletArray) {
                ball.update(dt);
                ...
                if(ball.isDestroyed())
                    enemyBulletArray.removeValue(ball, true);
            }
        }
    }


    public void updateEnemyFlying(float dt){
        if(!enemyHasSteeringStatBool) {
            if(!runningRight){
                this.b2body.setLinearVelocity(new Vector2(-0.9f, -0.4f));
                //System.out.println("EnemyB vel -1.2f -0.4f");
    ...
    public boolean getEnemyHasSteeringStateBool(){
        return enemyHasSteeringStatBool;
    }

    public void setEnemyHasSteeringStateBool(boolean value){

        this.enemyHasSteeringStatBool = value;
    }


    public void setSwoopingActionFlyby(boolean value){

    }

    public void setSwoopingActionFlyByTargetH(float targheight){

       this.actionFlyby_Height = targheight;
    }




    public void swoopingFlyby(){

        if(actionFlyby_Height > 0) {

            float current_height = this.b2body.getPosition().y;
            float smoothness = current_height - 0.05f;

            current_height = smoothness * current_height +
                   (1.0f - smoothness) * actionFlyby_Height;

            setPosition(b2body.getPosition().x - getWidth() / 2, current_height - getHeight() / 2);
            System.out.println("EnemyB fly by test...");
        }


        /**
         *  float target_height = 0.0f; // Ground Height = player height Y etc
         *  float current_height = 100.0f; // currentPosition = startPosition
         *  float smoothness = 0.95f; current_height_Stored - 0.05f
         *
         *  update() {
         *
         *      current_height = smoothness * current_height +
    ...
    public void reverseVelocity(boolean x, boolean y){
        ...
        System.out.println("reversVelocity @Override!!");
    ...
    public void die() {
        ...
        if (!isDead()) {
            ...
            b2body.setLinearVelocity(0f,0f); // stop all movement
            b2body.setGravityScale(1.0f); // gravity to normal
    ...
    public void setFlyingDirectionRight(boolean value){

        if(value){
            this.b2body.setLinearVelocity(new Vector2(1.2f, -0.4f));
        }
    }

    public void setFlyingDirectionLeft(boolean value){
